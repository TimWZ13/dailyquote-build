...
final class QuoteStore: ObservableObject {
    ...
    /// 收藏 ID 缓存，使 isFavorite 判定从 O(n) 降为 O(1)
    /// 始终与 favorites 数组保持同步。©️Trigin
    ...
    init() {
        ...
        self.allQuotes = allQ
        self.favorites = favs
        self.favoriteIDs = Set(favs.map { $0.id })
        ...
        NotificationCenter.default.addObserver(
            ...
            // 弱引用先在 Task 捕获列表中转换，避免逃逸捕获 self 的并发错误
            let boundSelf = self
            Task { @MainActor in
                boundSelf?.refreshFontSize()
            }
        }
    }
    ...
    func importFavorites(from data: Data, strategy: ImportStrategy = .merge) -> ImportSummary {
        ...
        do {
            ...
            case .replace:
                favorites = incoming
                favoriteIDs = Set(incoming.map { $0.id })
                saveFavorites()
                return ImportSummary(
